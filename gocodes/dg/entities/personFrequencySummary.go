...
// 车辆频率分析概述
type PersonFrequencySummary struct {
...
func (PersonFrequencySummary) TableName() string {
	return TableNamePersonFrequencySummary
}

func (vfa *PersonFrequencySummary) ToModel() *models.PersonFrequencySummary {
	return &models.PersonFrequencySummary{
		Uts:        vfa.Uts,
		Ts:         vfa.Ts,
		TaskID:     vfa.TaskID,
...
func PersonFrequencySummaryEntitiesToModels(list []*PersonFrequencySummary) []*models.PersonFrequencySummary {
	model := make([]*models.PersonFrequencySummary, 0)
	for _, l := range list {
